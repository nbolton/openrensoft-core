...
namespace Rensoft.Net
{
    public class FtpClient
    {
        ...
        {
            return Dns.GetHostEntry(hostName).AddressList.Single(a => a.AddressFamily == AddressFamily.InterNetwork);
        }
        ...
        public string[] GetFileList(string mask)
        {
            ...
            while (timeout > DateTime.Now)
            {
                ...
                this.message += encoding.GetString(buffer, 0, bytes);
        ...
        {
            this.Download(remoteFileName, remoteFileName, false);
        }
        ...
        {
            this.Download(remoteFileName, remoteFileName, resume);
        }
        ...
        {
            this.Download(remoteFileName, localFileName, false);
        }
        ...
        {
            Download(remoteFileName, new FileStream(localFileName, FileMode.OpenOrCreate), resume);
        }

        public void Download(string remoteFileName, Stream output)
        {
            Download(remoteFileName, output, false);
        }

        public void Download(string remoteFileName, Stream output, bool resume)
        {
            if (!this.loggedin) this.Open();

            this.BinaryMode = true;

            Debug.WriteLine("Downloading file " + remoteFileName + " from " + server + "/" + remotePath, "FtpClient");
            ...
            this.sendCommand("RETR " + remoteFileName);
        ...
        {
            Upload(new FileStream(fileName, FileMode.Open), fileName, resume);
        }

        /// <summary>
        /// Upload from a stream.
        /// </summary>
        public void Upload(Stream input, string targetFileName)
        {
            Upload(input, targetFileName, false);
        }

        /// <summary>
        /// Upload from a stream and set the resume flag.
        /// </summary>
        public void Upload(Stream input, string targetFileName, bool resume)
        {
            if (!this.loggedin) this.Open();
            ...
            {
                // different file size
                Debug.WriteLine("Overwriting " + targetFileName, "FtpClient");
                offset = 0;
            }
            else if (resume && input.Length == offset)
            {
                // file done
                input.Close();
                Debug.WriteLine("Skipping completed " + targetFileName + " - turn resume off to not detect.", "FtpClient");
                return;
            }
            ...
            this.sendCommand("STOR " + Path.GetFileName(targetFileName));

            if (this.resultCode != 125 && this.resultCode != 150) throw new FtpException(result.Substring(4));

            // Seek to start or where ever resume left off.
            ...
            Debug.WriteLine("Uploading file " + targetFileName + " to " + remotePath, "FtpClient");
        ...
        public void UploadDirectory(string path, bool recurse, string mask)
        {
            ...
            this.ChangeWorkingDirectory(rootDir);
        ...
        private string readLine()
        {
            while (true)
            {
                ...
                this.message += encoding.GetString(this.buffer, 0, this.bytes);
        ...
        private Socket createDataSocket()
        {
            ...
            try
            {
                ...
                ep = new IPEndPoint(getAddress(ipAddress), port);
                socket.Connect(ep);
            }
            catch (Exception ex)
            {
...
*/
        ...
        {
            LoginCallback ftpCallback = new LoginCallback(this.Open);
        ...
        {
            RemoveDirCallback ftpCallback = new RemoveDirCallback(this.RemoveDirectory);
            return ftpCallback.BeginInvoke(dirName, callback, null);
        }
        ...
        {
            ChangeDirCallback ftpCallback = new ChangeDirCallback(this.ChangeWorkingDirectory);
            return ftpCallback.BeginInvoke(dirName, callback, null);
        }
